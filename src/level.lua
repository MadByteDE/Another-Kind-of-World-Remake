...
local entities = {
    ...
    bug      = require("src.objects.bug"),
    exit     = require("src.objects.exit"),
    bomb     = require("src.objects.bomb"),
...
local function generateTileData(self)
    ...
    for y = 1, Game.screen.height / self.tileSize do
        tileData[y] = {}
        for x = 1, Game.screen.width / self.tileSize do

            local tile = self:getTile(x, y)
            for assKey, assVal in ipairs(Game.assets.tiles) do
...
local function createLevel(self)
    ...
    for y = 1, Game.screen.height / self.tileSize do
        ...
        for x = 1, Game.screen.width / self.tileSize do
            ...
            self.tiles[y][x] = Tile(self, tx, ty, Game.assets.getTile("back"))
        end
            ...
            for x = 1, Game.screen.width / self.tileSize do
                ...
                for assKey, assVal in ipairs(Game.assets.tiles) do
                    if (tileId == assKey) then
                        ...
                        if (assVal.type == "entity" and Game.current_scene == Game.scenes.Ingame) then
                            self:setTile(x, y, Tile(self, tx, ty, Game.assets.getTile("back")))
...
function Level:init(id)
    ...
    self.canvas   = love.graphics.newCanvas(Game.screen.width, Game.screen.height)
    self.objects  = Conta()
...
function Level:draw()
    ...
    self.objects:draw()
    if self.overlay and Game.current_scene.name == "Ingame" then
