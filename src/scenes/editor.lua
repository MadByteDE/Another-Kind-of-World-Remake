...
function Editor:init(id)
    ...
    self.currentTile = Game.assets.getTile("wall")
    ...
    local titleBox = self.gui:add("textbox", Game.screen.width/2-36, 2, data)
end
...
function Editor:logic(dt)
    ...
    elseif mouse.button == 2 then
        self.currentTile = Game.assets.getTile(self.level:getTile(tx, ty).name)
    end
end
...
function Editor:render()
    ...
    if self.currentTile.quad then
        love.graphics.draw(Game.assets.spritesheet, self.currentTile.quad, tx*tw-tw, ty*tw-tw)
    end
    Game.assets.print("TAB - Switch to game\nLMB/RMB - Place/Pick tile\nSpace - Play level", 3, 2, {rgba={1, 1, 1, .075}, width=100})
end
...
function Editor:keypressed(key)
    ...
    elseif key == "space" and not self.gui.selectedElement then
        Game.screen:transition(function()
            ...
            Game:switchScene("Ingame")
            Game.current_scene:init(self.level.id, true)
        end, 1)
    end
end
